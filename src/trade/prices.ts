...
import type { OpenClawPriceSnapshot } from "./openclawDecision";
...
export function attachPricesMethods(TradeClass: new (...args: any[]) => any) {
    ...
    TradeClass.prototype.updatePrices = function (
        ...
        const snapshot: OpenClawPriceSnapshot = {
            ts: Date.now(),
            up_buy: upBuyPrice,
            up_sell: upSellPrice,
            down_buy: downBuyPrice,
            down_sell: downSellPrice,
        };
        if (globalThis.__CONFIG__?.openclaw?.enabled) {
            this.openclawHistory?.push(snapshot);
        }
        ...
        if (now - this.lastStatusLogAt >= 3000) {
            ...
            console.log(divider("MARKET"));
            console.log(
                `tMinus=${remaining_time}s/${this.marketTime}s | engine=${GLOBAL_TX_PROCESS.current === TxProcess.Working ? "BUSY" : "IDLE"} | trend=${trend} | position=${position}`
            );
            console.log(
                `UP   buy/sell=${this.upBuyPrice.toFixed(2)}/${this.upSellPrice.toFixed(2)} | spread=${upSpread.toFixed(2)} | upRatio=${upPriceRatio.toFixed(2)}`
            );
            console.log(
                `DOWN buy/sell=${this.downBuyPrice.toFixed(2)}/${this.downSellPrice.toFixed(2)} | spread=${downSpread.toFixed(2)}`
            );
            console.log(
                `Signal | timeRatio=${remainingTimeRatio.toFixed(2)} | score=${signalScore.toFixed(2)}`
            );
            this.displayBalance();
