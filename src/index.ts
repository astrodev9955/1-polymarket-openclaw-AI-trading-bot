...
function logStartupBanner() {
  ...
  const bar = "━".repeat(68);
  console.info(bar);
  console.info(`  OPENCLAW  ·  POLYMARKET AI TRADING BOT  ·  ${coin}  ·  ${marketConfig.minutes}m`);
  console.info(bar);
}
...
async function main() {
  ...
  logger.info(
    `OpenClaw: ${globalThis.__CONFIG__.openclaw.enabled ? "ENABLED" : "disabled"} | mode=${globalThis.__CONFIG__.openclaw.mode} | min_edge_bps=${globalThis.__CONFIG__.openclaw.min_edge_bps} | max_spread_bps=${globalThis.__CONFIG__.openclaw.max_spread_bps} | lookback_points=${globalThis.__CONFIG__.openclaw.lookback_points}`
  );
